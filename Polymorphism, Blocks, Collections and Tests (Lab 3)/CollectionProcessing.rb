...
# Построение двух списков: 
# L1 - уникальные элементы входного списка
# L2 - количество вхождений элементов L1 во входном списке (по номеру)
def build_lists(array)
	l1 = array.uniq # уникальные элементы array
	l2 = l1.map{|element| array.count(element)}
	return l1, l2
end

# Ввод номера метода с клавиатуры
puts "Выберите метод:\n1 - Проверка на глобальный максимум\n2 - Проверка на локальный минимум\n3 - Циклический сдвиг влево на 1\n4 - Вывод элементов сначала с четными индексами, затем - с нечетными\n5 - Вывод уникальных элементов и количества их повторений"
print "Введите номер: "
...
when 5
	...
	input = gets.chomp
	array = input.split.map{|x| x.to_i}
	l1, l2 = build_lists(array)
	puts "Уникальные элементы: #{l1}"
	puts "Количество вхождений элементов в список: #{l2}"
end